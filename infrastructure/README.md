# Pricofy Geocode DE - Infrastructure

This directory contains the AWS CDK infrastructure-as-code for deploying the **pricofy-geocode-de** service.

## Overview

The infrastructure is defined using AWS CDK (Cloud Development Kit) and deploys:

- **Lambda Function**: Single function (`pricofy-geocode-de-{env}`) with internal routing
...
- **Lambda Function**: `pricofy-geocode-de-{env}`
...
From the root of `pricofy-geocode-de`:
...
npm install
npm run build
cdk deploy GeocodeDeStack-dev --context environment=dev
```
...
CloudWatch Logs:
```bash
aws logs tail /aws/lambda/pricofy-geocode-de-dev --follow
```
