/**
 * Geocode DE End-to-End Tests
 * 
 * Comprehensive integration tests for the pricofy-geocode-de Lambda function.
...
import { GeocodeDEClient } from './client';
...
// Initialize client
const client = new GeocodeDEClient();

describe('Geocode DE E2E Tests', () => {
  beforeAll(() => {
    console.log('🚀 Starting Geocode DE E2E Tests');
  ...
  afterAll(() => {
    console.log('\n🏁 Geocode DE E2E Tests Completed');
  });
  ...
  describe('Reverse Geocode', () => {
    test('should reverse geocode Madrid coordinates', async () => {
      ...
      expect(result).toHaveProperty('province');
      expect(result).toHaveProperty('country', 'Deutschland');
  ...
  describe('Edge Cases - Reverse Geocode', () => {
    test('should handle coordinates in Canary Islands', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(response.body.success).toBe(true);
      expect(response.body.country).toBe('Deutschland');
    ...
    test('should handle coordinates outside Spain (should return error or closest)', async () => {
      ...
      // Should either return error or closest German location
      if (response.statusCode === 400) {
        ...
        expect(response.statusCode).toBe(200);
        expect(response.body.country).toBe('Deutschland');
        console.log(`   ✅ Returned closest German location`);
      }
    }, 30000);
  });
