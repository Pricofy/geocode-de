...
SERVICE_NAME = pricofy-geocode-de
STACK_SERVICE = PricofyGeocodeDeStack
LAMBDA_GEOCODE = pricofy-geocode-de
...
test-geocode: ## Test geocode Lambda (ENV=dev)
	...
	@aws lambda invoke \
		...
		--payload $(shell echo '{"body":"{\"operation\":\"geocode-by-postal\",\"postalCode\":\"10115\"}"}' | base64 -w 0) \
		response.json
